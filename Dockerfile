...
RUN git clone https://gitlab.linphone.org/BC/public/flexisip.git --recursive --depth 1 && \
    cd flexisip && ./prepare.py flexisip -DENABLE_TRANSCODER=ON -DENABLE_SNMP=YES -DENABLE_DOC=OFF -DENABLE_PROTOBUF=YES -DCMAKE_PREFIX_PATH=/opt/belledonne-communications -DCMAKE_INSTALL_PREFIX=/opt/belledonne-communications -DSYSCONF_INSTALL_DIR=/etc && make && \
